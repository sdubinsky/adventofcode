...
checksum [] = 0
checksum line =
    let max = foldr (\x m -> if x > m then x else m) 0 line in
    let min = foldr (\x m -> if x < m then x else m) 1000 line in
    (max - min)
